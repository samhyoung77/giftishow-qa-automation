...
# import undetected_chromedriver as uc  # ARM64 Mac 호환성 문제로 비활성화
...
def driver(config):
    """
    WebDriver fixture (세션 범위)
    전체 테스트 세션 동안 하나의 브라우저 인스턴스 사용
    - 브라우저 깜빡임 방지
    - 테스트 실행 속도 향상
    """
    ...
    try:
        if config.BROWSER == "chrome":
            ...
            prefs = {
                ...
                "profile.default_content_setting_values.clipboard": 2,  # 클립보드 차단
            }
...
def pytest_collection_modifyitems(items):
    """테스트 실행 순서 조정 - 로그인 테스트 먼저 실행"""
    login_tests = []
    other_tests = []

    for item in items:
        if "test_login" in item.fspath.basename:
